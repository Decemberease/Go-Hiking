...
struct BikeRideListView: View {
    ...
    @State var sortDescriptor = NSSortDescriptor(keyPath: \BikeRide.hikingTime, ascending: false)
        ...
        if (bikeRides.count > 0) {
            List {
                ForEach(bikeRides) { bikeRide in
                    NavigationLink(destination: SingleBikeRideView(bikeRide: bikeRide, navigationTitle: MetricsFormatting.formatDate(date: bikeRide.hikingStartTime))) {
                        // Bike ride list cell
                        VStack(spacing: 10) {
                            HStack {

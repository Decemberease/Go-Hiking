...
class ActivityAwardsViewModel: ObservableObject {
    ...
    private var initialRecords = HikingRecords.shared
    
    @Published var progressValues: [CGFloat] = [CGFloat].init(repeating: 0.0, count: 6)
    @Published var unlockedIcons = HikingRecords.shared.unlockedIcons
    ...
    @Published var records: HikingRecords? {
        willSet {
            // Update published values when records change
            unlockedIcons = HikingRecords.shared.unlockedIcons
            for index in 0..<HikingRecords.awardValues.count {
                ...
                // Single route awards
                    ...
                    let distance = self.initialRecords.longestHikingDistance
                    progressFloat = CGFloat(distance/HikingRecords.awardValues[index]) > 1.0 ? 1.0 : CGFloat(distance/Records.awardValues[index])
                ...
                // Cummulative route awards
                    ...
                    let distance = self.initialRecords.totalHikingDistance
                    progressFloat = CGFloat(distance/HikingRecords.awardValues[index]) > 1.0 ? 1.0 : CGFloat(distance/Records.awardValues[index])
                    progressValues[index] = progressFloat
                    let roundedProgress = round(progressFloat * 10000) / 100.0
                    progressStrings[index] = "\(roundedProgress)% Complete"
    ...
    // Total Hiking routes changes each time the records change
    init(recordsPublisher: AnyPublisher<Int, Never> = HikingRecords.shared.$totalHikingRoutes.eraseToAnyPublisher()) {
    ...
    func getAwardName(index: Int, usingMetric: Bool) -> String {
        ...
        if (index < 3) {
            return "Hike at least \(distanceString) in a single route"
        }
        else {
            return "Hike a total of at least \(distanceString)"
        }
    }

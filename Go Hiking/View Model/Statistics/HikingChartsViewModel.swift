...
class HikingChartsViewModel: ObservableObject {
    ...
    func setPastWeekFormattedData() {
        ...
        if let date = endOfToday {
            for entry in pastWeekData {
                let dateDiff = entry.hikingStartTime.fullDistance(from: date, resultIn: .day, calendar: calendar) ?? 0
                if (dateDiff < 7 && dateDiff >= 0) {
                    self.pastData[0][6 - dateDiff] += entry.hikingDistance
                    self.pastData[3][6 - dateDiff] += entry.hikingTime
        ...
        if let date = endOfToday {
            for entry in past5WeeksData {
                ...
                if (dateDiff < 35 && dateDiff >= 0) {
                    self.pastData[1][4 - (dateDiff / 7)] += entry.hikingDistance
                    self.pastData[4][4 - (dateDiff / 7)] += entry.hikingTime
        ...
        if let date = endOfToday {
            for entry in past30WeeksData {
                ...
                if (dateDiff < 209 && dateDiff >= 0) {
                    self.pastData[2][5 - (dateDiff / 35)] += entry.hikingDistance
                    self.pastData[5][5 - (dateDiff / 35)] += entry.hikingTime

...
extension BikeRide {
    ...
    @nonobjc public class func fetchRequestsWithDateRanges() -> [NSFetchRequest<BikeRide>?] {
        ...
        if let date1 = dateFrom1, let date2 = endOfToday {
            ...
            let fromPredicate = NSPredicate(format: "%@ <= %K", date1 as NSDate, #keyPath(BikeRide.hikingStartTime))
            let toPredicate = NSPredicate(format: "%K <= %@", #keyPath(BikeRide.hikingStartTime), date2 as NSDate)
    ...
    static func sortByDate(list: [BikeRide], ascending: Bool) -> [BikeRide] {
        ...
        let bikeRideDates: [Date] = list.map { $0.hikingStartTime }
